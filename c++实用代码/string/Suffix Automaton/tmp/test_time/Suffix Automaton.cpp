...
struct State{
	State *suf,*son[CH];int sum[CH],dep,ans,curlen;
...
void extend(char c){
	...
	if (!p)np->suf=root;
		...
		if (p->dep+1==q->dep)np->suf=q;
			...
			int s=0;
			while (p&&p->son[c]==q)++p->sum[c],p->son[c]=nq,p=p->suf,++s;
			S+=s;printf("dep=%d s=%d S=%d\n",np->dep,s,S);
		}
	}
	last=np;
}
...
int main()
{
	...
	while(scanf("%s",str)!=EOF){
		...
		for (char *pt=str;*pt;++pt){
			...
			if (now->son[c])now=now->son[c],now->curlen=max(now->curlen,++l);
		}
	...
	int max=0;
	for (State *x=StatePool;x!=cur;++x)
		for (int i=0;i<CH;++i)if (x->sum[i]>max)max=x->sum[i];
	printf("max=%d\n",max);
	//printf("time=%d\n",clock()-t1);
	//system("pause");for (;;);
	return 0;
