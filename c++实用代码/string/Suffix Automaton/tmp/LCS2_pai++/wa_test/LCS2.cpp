...
#define CH 26
struct State{
	State *suf,*son[CH];int dep,l[9],mnlen;
...
int main()
{
	...
	for (int i=cur-StatePool;i;--i){
		...
		ans=max(ans,tmp);
		x->mnlen=tmp;
		//if (tmp>x->dep)printf("wawawa%d %d\n",tmp,x->dep);
	}
	for (int i=0;i<id;++i)
		for (State *x=StatePool;x!=cur;++x)printf("l=%d\n",x->l[i]);
	for (State *x=StatePool;x!=cur;++x)printf("all=%d\n",x->mnlen);
	printf("%d\n",ans);
	//system("pause");for (;;);
	return 0;
}
