...
inline ll get_pair(int x,int y){return (ll)x*N+y;}
struct LinkCutTree{
	struct node{
		int rev;
		node *l,*r,*p;
...
struct EulerTourTree{
	...
	node *newnode(int x,int y){
		...
		node *res=stack[stack1--];
		ini_node(res);res->id=res-a;
		res->x=x;res->y=y;
		return res;
	}
	void delnode(node *x){stack[++stack1]=x;}
	node *getedge(int x,int y){
		node* ans=M.find(get_pair(x,y));
		if (ans!=M.null)return ans;
		else return null;
	}
	EulerTourTree(LinkCutTree &LCtree,hash<ll,node*> &Hash,int n=0):LC(LCtree),M(Hash){
		n+=5;n*=3;stack1=-1;stack=new node*[n];
		a=new node[n];null=new node;ini_node(null);
		null->id=-1;null->x=null->y=-1;
	...
	void build(node **x,int l,int r){
		int mid=(l+r)>>1,tmp;
		if (l<mid){
			build(x,l,mid-1);tmp=(l+mid-1)>>1;
			x[mid]->l=x[tmp];x[tmp]->p=x[mid];
		}else x[mid]->l=null;
		if (r>mid){
			build(x,mid+1,r);tmp=(mid+1+r)>>1;
			x[mid]->r=x[tmp];x[tmp]->p=x[mid];
		}else x[mid]->r=null;
		x[mid]->update();
	}
	node *head(node *p){
	...
	void link(int u,int v){
		...
		node *u_v=newnode(u,v),*v_u=newnode(v,u); LC.link(u,v);
		M.insert(get_pair(u,v),u_v);M.insert(get_pair(v,u),v_u);
		node *T1_1,*T1_2,*T2_1,*T2_2; pair<node*,node*> tmp;
		tmp=split_l(getedge(u,u));T1_1=tmp.first;T1_2=tmp.second;
		tmp=split_l(getedge(v,v));T2_1=tmp.first;T2_2=tmp.second;
		merge(merge(merge(T1_2,T1_1),u_v),merge(merge(T2_2,T2_1),v_u));
	}
	void cut(int u,int v){
		...
		M.del(get_pair(u,v));M.del(get_pair(v,u));
	}
	void add(int x,ll y){
		int fa=LC.Parent(x);node *now;
		if (fa==LC.getid(LC.null)){
			splay(now=getedge(x,x));inc(now,y);return;
		}
		else {
			node *fa_x=getedge(fa,x),*x_fa=getedge(x,fa);
			int l_r=get_l_r(fa_x,x_fa); if (l_r)swap(x_fa,fa_x);
			node *T1,*T2,*T3;
			T1=split_r(fa_x).first;split_l(fa_x);
			T2=split_r(x_fa).first;T3=split_l(x_fa).second;
			T1=merge(T3,T1);
			if (!l_r)inc(T2,y);else inc(T1,y);
			merge(merge(T2,x_fa),merge(T1,fa_x));
		}
	}
	ll Query(int x){
		int fa=LC.Parent(x);node *now;
		if (fa==LC.getid(LC.null)){
			splay(now=getedge(x,x));return now->sum;
		}
		else {
			node *fa_x=getedge(fa,x),*x_fa=getedge(x,fa);
			int l_r=get_l_r(fa_x,x_fa); if (l_r)swap(x_fa,fa_x);
			ll res; node *T1,*T2,*T3;
			T1=split_r(fa_x).first;split_l(fa_x);
			T2=split_r(x_fa).first;T3=split_l(x_fa).second;
			T1=merge(T3,T1);
...
hash<ll,EulerTourTree::node*> H(N*5,N*7);
EulerTourTree ET(LC,H,N);
struct Edge{
...
void copy(EulerTourTree &ET){
	...
	for (int i=1;i<=c.len;++i){
		x[i]=ET.newnode(c.edgex[i],c.edgey[i]);
		ET.M.insert(get_pair(c.edgex[i],c.edgey[i]),x[i]);
		if (c.edgex[i]==c.edgey[i])
			x[i]->exist=1,x[i]->key=w[c.edgex[i]];
		x[i]->update();
		//if (i>1)ET.merge(x[i-1],x[i]);
	}
	ET.build(x,1,c.len);
}
void copy1(EulerTourTree &ET){
	for (int i=1;i<=n;++i){
		ET.LC.a[i].p=ET.LC.a[i].l=ET.LC.a[i].r=ET.LC.null;
		ET.LC.getnode(i)->update();
	}
	static EulerTourTree::node *x[N];
	for (int i=1;i<=n;++i){
		x[i]=ET.newnode(i,i);
		ET.M.insert(get_pair(i,i),x[i]);
		x[i]->exist=1;x[i]->key=w[i];x[i]->update();
	}
	for (int i=1;i<=c.len;++i)if (c.edgex[i]!=c.edgey[i])ET.link(c.edgex[i],c.edgey[i]);
	for (int i=1;i<=c.len;++i)if (c.edgex[i]!=c.edgey[i])ET.cut(c.edgex[i],c.edgey[i]);
	for (int i=1;i<=c.len;++i)if (c.edgex[i]!=c.edgey[i])ET.link(c.edgex[i],c.edgey[i]);
	ET.Changeroot(1);
}
void init(){
