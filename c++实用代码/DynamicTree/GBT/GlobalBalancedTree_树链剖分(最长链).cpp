...
struct GBT{
	...
	int dep[N],f[N],fa[N],heavy[N]; char Map[N][2];
	void clear(){
	...
	node *build(int start,int siz,node *head,int dep){
		if (!siz)return NULL;
		int mid,s=1,now=start;
		while (s+s<siz)now=heavy[now],++s;
		node *x=getnode(now);x->head=head;x->dep=dep;
		if ((x->l=build(start,s-1,head,dep+1))!=NULL)x->l->p=x;
	...
	void bfs(int root,int n){
		...
		for (int i1=t;i1;--i1){
			int x=Q[i1];heavy[x]=0;f[x]=0;
			for (edge *i=son[x];i;i=i->next)if (i->link!=fa[x]){
				int f1=f[i->link];
				if (f1>f[x])f[x]=f1,heavy[x]=i->link;
			}
			++f[x];
		}
		...
		for (int i1=1;i1<=n;++i1){
			...
			if (!fa[i]||heavy[fa[i]]!=i){
				int siz=0;for (int now=i;now;now=heavy[now])++siz;
				node *x=build(i,siz,getnode(i),getnode(fa[i])->dep+1);
