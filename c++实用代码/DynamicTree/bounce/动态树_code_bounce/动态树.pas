...
type
  splaya=record
    l,r,key,size,p,min:longint;
    end;
var
  a:array[0..mm]of splaya;
  father:array[0..mm]of longint;
  n,m,x,y,z,ll,i,j,k,op:longint;

  procedure update(x:longint);inline;   //更新节点
    begin
...
procedure zag(x:longint);inline;   //右旋
  var
    y,z:longint;
    begin
    y:=a[x].p;
...
procedure Splay(x:longint);inline;
  ...
  procedure expose(x:longint);inline;  //将从x到root(x)中所有边设为实边
    begin
    while true do
      begin
      x:=get_head(x);
  ...
  function findroot(x:longint):longint;inline;  //包含节点x的树的根
    begin
    expose(x);
    while a[x].l>0 do x:=a[x].l;
    exit(x);
