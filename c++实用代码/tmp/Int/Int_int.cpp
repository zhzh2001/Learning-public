...
class Int{
	...
	friend Int operator +(Int A,Int B){
		...
		Int res;res.set_base(A.base); size_t len=A.len>B.len?A.len:B.len;
	...
	friend Int operator *(Int A,Int B){
		...
		Int res;res.set_base(A.base); size_t len=A.len+B.len;
	...
	friend pair<Int,Int> divide(Int A,Int B){
		...
		for (int i=A.len-1;i>=0;--i){
			...
			while (l<r){
				mid=(l+r+1)>>1;
				if (small(B*Int(mid),D+Int(1)))l=mid;
					else r=mid-1;
			}
			C.a[i]=l;D=D-B*Int(l);
		}
	...
	Int& operator /=(Int B){*this=*this/B;return *this;}
	Int& operator %=(Int B){*this=*this%B;return *this;}
	Int& operator ++(){*this=*this+Int(1);return *this;}
	Int& operator --(){*this=*this-Int(1);return *this;}
	Int operator ++(int){Int res(*this);*this=*this+Int(1);return res;}
	Int operator --(int){Int res(*this);*this=*this-Int(1);return res;}
	...
	void set_basel(int _l){
		size_t l=0,_base=1;while (_l)_base*=10;set_base(_base);
	}
	void read(){
	...
	operator value(){
		value x=0;
		for (int i=len-1;i>=0;--i)x=x*base+a[i];
		return x;
	}
	private:
...
int main()
{
	...
	a=123;b=456;c=a+b;
	c.println();
	for (Int i=1;i<=Int(10);)(i++).println();
	for (Int i=1;i<=Int(10);)(++i).println();
