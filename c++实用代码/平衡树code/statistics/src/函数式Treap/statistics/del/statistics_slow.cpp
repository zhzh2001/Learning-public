...
struct Treap{
	...
	node *del(node *x,int key){
		if (x==NULL)return NULL;
		if (key<x->key)return merge(del(x->l,key),merge(newnode(x->key),x->r));
		else if (key>x->key)return merge(merge(x->l,newnode(x->key)),del(x->r,key));
...
};int Treap::a1=0;Treap::node *Treap::A=new Treap::node[BUFFER_SIZE];
Treap a;
int main(){
