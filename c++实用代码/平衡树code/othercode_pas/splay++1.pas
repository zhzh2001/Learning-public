...
function getmin(tt:int64):int64;inline;  //最小节点
var
	t:int64;
begin
	t:=tt;
...
function getmax(tt:int64):int64;inline;  //最大节点
var
	t:int64;
begin
	t:=tt;
...
begin
  ...
  begin
  //assign(input,'seqope.in'); reset(input);
  //assign(output,'seqope.out'); rewrite(output);
  a[0].min:=10000000000;
  ...
  for i:=1 to q do
    ...
    if ch='Q' then begin
      ...
      writeln(a[a[a[root].r].l].sum);
      end
      else begin
      readln(z);
      update1(a[a[root].r].l,z);
      update(a[root].r);
      update(root);
      end;
    end;
