...
#define oo 50000000
int son[maxn],q[maxn],d[maxn],pre[maxn],inq[maxn],visit[maxn];
...
int dfs(int x,int flow){
    if (x==T)return flow;
    visit[x]=1;int l=x,p;
    for (p=son[i];p!=-1&&l;p=next[p])
        if (cap[p]&&!visit[link[p]]&&d[link[p]]==d[x]+cost[p]){
            int t=dfs(link[p],l>cap[p]?cap[p]:l);
            maxflow+=t,mincost+=t*cost[p],cap[p]-=t,
            cap[opp[p]]+=t,l-=t;
            }
