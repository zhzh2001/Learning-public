...
struct Tree{
	...
	int Qsum(int t,int l,int r,int ll,int rr){
		...
		if (rr<=mid)return a[t].L?Qsum(a[t].L,l,mid,ll,rr):0;
			...
			else return (a[t].L?Qsum(a[t].L,l,mid,ll,mid):0)+
						(a[t].R?Qsum(a[t].R,mid+1,r,mid+1,rr):0);
	}
};
Tree T;
struct Tree1{
	...
	int Qmax(int t,int l,int r,int ll,int rr){
		...
		if (rr<=mid)return a[t].L?Qmax(a[t].L,l,mid,ll,rr):-oo;
			...
			else return max((a[t].L?Qmax(a[t].L,l,mid,ll,mid):-oo),
						(a[t].R?Qmax(a[t].R,mid+1,r,mid+1,rr):-oo));
	}
};
Tree1 T1;
int main()
