...
int main(){
	...
	for (int i=0;i<N;++i){
		end=index_lib::varint_encode_uint32(a[i],end);
		//printf("end=%d\n",end);
	}
	for (uint8_t *i=c;i!=end;++i)printf("%c",*i);
	fclose(stdout);*/
	freopen("1.out","r",stdin);
	//freopen("2.in","w",stdout);
	uint8_t ch;end=c;char ch1;
	...
	while ((end=index_lib::varint_decode_uint32(end,a))!=NULL){
		//printf("RP\n");
		printf("%u\n",a[0]);
		system("pause");//for (;;);
	}
