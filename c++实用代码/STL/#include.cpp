...
inline ll P(int n,int m){
	if (n>m)return 0;
	ll res=1;
	for (int i=1;i<=n;++i)res=res*(m-i+1);
	return res;
}
inline ll fac(int n){ll res=1;while (n)res*=n--;return res;}
...
struct graph{
	...
	void tarjan(int *_num){
		...
		for (int i=1;i<=n;++i)if (!dfn[i])tarjan_f(i);
...
struct BitSet{
	...
	BitSet& operator =(const BitSet &y){
		memcpy(a,y.a,sizeof(uint)*size);return *this;
	}
	...
	void flip(uint x){a[x>>W]^=1<<(x&mask);}
