...
plane pf(0,0,1,0);
point3 pp[1120],q[1120],s;
polygon pl;
int main()
{
    //pf.pttoplane(point3(0,1,1)).output();
    int i,j,k;
    double a,b,c,d;
    int n,m;
    scanf("%d%d",&n,&m);
    ...
    for (j=0;j<m;j++)
    {
        ...
        for (i=0;i<n;i++)
        {
            pp[i]=ps.pttoplane(q[i]);
            //p[i]=q[i];
            //p[i].output();
            //q[i].output();
        }
        if (!ps.crossplane(pf,o))
        {
            for (i=0;i<n;i++)
            {
                pl.add((point(pp[i].x,pp[i].y)));
            }
        }
        else 
        {
            double r=ps.angleplane(pf);
            for (i=0;i<n;i++)
            {
                point3 ppp=o.rotate(pp[i],r);
                pl.add((point(ppp.x,ppp.y)));
                //pl.p[i].output();
            }
        }
        //pl.p[0].output();
