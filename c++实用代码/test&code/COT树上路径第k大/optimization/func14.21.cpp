...
struct func_segment_tree{
	...
	int query(int x,int y,int z,int z1,int l,int r,int k){
		if (l==r) return l;
		int mid=(l+r)/2,S=size[L[x]]+size[L[y]]-size[L[z]]-size[L[z1]];
...
inline void bfs()
{
	...
	for (int i=1;i<=n;i++) deep[i]=b[i]=0; deep[1]=1;
	while (h<t){
		int i=f[++h];
	...
	T.build(root[0],1,m);
	for (int k=1;k<=t;k++) T.modify(root[fa[f[k]]],root[f[k]],1,m,w[f[k]]);
}
...
int main()
{
	...
	read(n),read(Q); m=0;
	for (int i=1;i<=n;i++) read(dis[i].x),dis[i].id=i,val[i]=dis[i].x;
	...
	while (Q--){
		...
		printf("%d\n",v[T.query(root[x],root[y],root[z],root[z1],1,m,k)]);
		}
	printf("%lf\n",clock()-t1);
	return 0;
}
