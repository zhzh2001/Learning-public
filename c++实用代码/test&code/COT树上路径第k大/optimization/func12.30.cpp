...
struct note{int x,id;} dis[N];
	...
	void modify(int t,int &t1,int l,int r,int x){
		int mid=(l+r)>>1;
		if (l==r){L[t1]=R[t1]=0; size[t1]=size[t]+1; return;}
		if (x<=mid) L[t1]=++tot,modify(L[t],L[t1],l,mid,x),R[t1]=R[t];
		else R[t1]=++tot,modify(R[t],R[t1],mid+1,r,x),L[t1]=L[t];
		size[t1]=size[L[t1]]+size[R[t1]];
		}
...
inline void bfs()
{
	...
	build(root[0],1,m);
	for (int k=1;k<=t;k++) root[f[k]]=++tot,modify(root[fa[f[k]]],root[f[k]],1,m,w[f[k]]);
}
