...
struct note{int x,id;} dis[N];
	...
	int query(int x,int y,int z,int l,int r,int k){
		if (l==r) return l;
		int mid=(l+r)>>1,S=size[L[x]]+size[L[y]]-(size[L[z]]<<1)-(W>=l&&W<=mid);
		if (S>=k) return query(L[x],L[y],L[z],l,mid,k);
		else return query(R[x],R[y],R[z],mid+1,r,k-S);
		}
...
int main()
{
	...
	while (Q--){
		...
		printf("%d\n",v[query(root[x],root[y],root[z1],1,m,k)]);
		}
	printf("%lf\n",clock()-t1);
	return 0;
}
