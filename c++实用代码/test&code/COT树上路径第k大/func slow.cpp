...
int son[N],next[N*2],ed[N*2],S;
struct note{int x,id;} dis[N];
	int size[N*25],L[N*25],R[N*25],left[N*25],right[N*25];
	void build(int &t,int l,int r){
		if (!t) t=++tot; size[t]=0; left[t]=l; right[t]=r; int mid=(l+r)>>1;
		if (l<r) build(L[t],l,mid),build(R[t],mid+1,r);
		}
	void modify(int t,int &t1,int l,int r,int x){
		t1=++tot; int mid=(l+r)>>1; left[t1]=left[t]; right[t1]=right[t];
	...
	int query(int x,int y,int z,int z1,int k){
		if (left[x]==right[x]) return left[x]; S=size[L[x]]+size[L[y]]-size[L[z]]-size[L[z1]];
		if (S>=k) return query(L[x],L[y],L[z],L[z1],k);
		else return query(R[x],R[y],R[z],R[z1],k-S);
		}
...
int main()
{
	...
	while (Q--){
		...
		printf("%d\n",v[query(root[x],root[y],root[z],root[z1],k)]);
		}
	printf("%lf\n",clock()-t1);
	return 0;
}
