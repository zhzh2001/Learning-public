...
struct T{int x[K],min[K],max[K],id,split;}f[N*4];
int n,k,cmp_k,p[K],s;
pair<int,int> A[M+1];
...
void build(int t,int l,int r){
	...
	f[t].split=cmp_k=d;sort(a+l,a+r+1,cmp);
	int mid=(l+r)>>1; f[t].id=a[mid].id;
	for (int i=0;i<k;++i)f[t].min[i]=f[t].max[i]=f[t].x[i]=a[mid].x[i];
	if (l==r)return;
	if (l<=mid-1)build(t*2,l,mid-1),update(t,t*2);
	if (mid+1<=r)build(t*2+1,mid+1,r),update(t,t*2+1);
}
...
void ask(int t,int l,int r){
	if (l>r)return;int d=f[t].split;
	upd(make_pair(dist(f[t].x,p),t));
	if (l==r||dist(p,f[t].min,f[t].max)>A[s].first)return;
	int mid=(l+r)>>1;
	if (p[d]<f[t].x[d])ask(t*2,l,mid-1),ask(t*2+1,mid+1,r);
	else ask(t*2+1,mid+1,r),ask(t*2,l,mid-1);
}
int main(){
