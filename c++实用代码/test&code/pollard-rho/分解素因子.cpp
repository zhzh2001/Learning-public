...
int pri[L] = {2, 3, 5};
//Miller_Rabin大素数判断
inline bool Miller_Rabin(register bigint n)
{
	if(n < 2) return false;
	if(n == 2) return true;
	if(!(n & 1)) return false;
	register bigint k = 0 , i , j , m , a;
	m = n - 1;
	while(!(m & 1)) m >>= 1, k++;
	for(i = 0 ; i < L ; i ++)
	{
...
bigint queue[100005];
void calc(){
	int h,t,N=1;/*scanf("%d",&N);*/bigint p;
	h=1;t=N;
...
int main()
{
	...
	while (1){
		zzy.clear();calc();
		int s=0;
		for (map<bigint,int>::iterator dhh=zzy.begin();dhh!=zzy.end();++dhh)
			++s;
		//printf("time=%d\n",clock()-t1);
		printf("Case #%d: %d\n",++i1,s);
		for (map<bigint,int>::iterator dhh=zzy.begin();dhh!=zzy.end();++dhh)
			for (int i=1;i<=dhh->second;++i)cout<<dhh->first<<"\n";
	}
	system("pause");for (;;);
	return 0;
}
