...
struct hash{
	...
	//int gethash(int x,int y){return (abs((x<<16)+y>>4+1)*434245LL)&((1<<24)-1);}
	int gethash(int x,int y){return abs((y<<16)+x+1)%pp;}
...
int GetLCA(int x,int y){
    int i;
    if (deep[x]>deep[y])
	 for (i=20;i>=0;--i){++dhh3;if (deep[x]-(1<<i)>=deep[y])x=F[x][i];}
		else for (i=20;i>=0;--i){++dhh3;if (deep[y]-(1<<i)>=deep[x])y=F[y][i];}
    for (i=20;i>=0;--i){++dhh3;
	  if (F[x][i]!=F[y][i]){
		x=F[x][i];y=F[y][i];
	  }
...
void dfs(int from,int x,int pre){
	...
	if (X[x]){
	  ...
	  L[tmp]=ans;
	  for (vector<int>::iterator i=son[x].begin();i!=son[x].end();++i){++dhh;
		if (H.find(tmp,*i)==-1)H.insert(tmp,*i,p[*i]);}
	  for (vector<int>::iterator i=son[from].begin();i!=son[from].end();++i){++dhh;
		if (H.find(tmp,*i)==-1)H.insert(tmp,*i,p[*i]);}
	  }
	for (int i=e.son[x];i;i=e.next[i])
	   if (e.link[i]!=pre){
...
int main()
{
	...
	//for (int wei=1;wei<=10000;++wei)
    {
	    ...
	    for (Fx=x;!X[Fx];Fx=f[Fx])++dhh3;
	    for (Fy=y;!X[Fy];Fy=f[Fy])++dhh3;
	    int tmp=GetLCA(Fx,Fy);
	    //printf("nie%d %d\n",Fx,Fy);
	    if (deep[tmp]<deep[Fx]&&deep[tmp]<deep[Fy]){
		 int rank=getrank(X[Fx],X[Fy]);ans=L[rank];
		 for (int tmp=x;tmp!=Fx;tmp=f[tmp]){++dhh3;
			if (H.find(rank,w[tmp])==0)++ans;
			H.inc(rank,w[tmp]);
			}
		 for (int tmp=y;tmp!=Fy;tmp=f[tmp]){++dhh3;
			if (H.find(rank,w[tmp])==0)++ans;
			H.inc(rank,w[tmp]);
			}
		 for (int tmp=x;tmp!=Fx;tmp=f[tmp])++dhh3,H.dec(rank,w[tmp]);
		 for (int tmp=y;tmp!=Fy;tmp=f[tmp])++dhh3,H.dec(rank,w[tmp]);
		 //printf("%d\n",ans);
		 continue;
		 }
	    if (Fx!=Fy){
		 ...
		 int zzy=GetLCA(Fx,y);
		 for (int tmp=y;tmp!=zzy;tmp=f[tmp]){++dhh3;
			if (H.find(rank,w[tmp])==0)++ans;
			H.inc(rank,w[tmp]);
			}
		 for (int tmp=F[zzy][0];deep[tmp]>=deep[Fy];tmp=f[tmp]){++dhh3;
			H.dec(rank,w[tmp]);
			if (H.find(rank,w[tmp])==0)--ans;
			}
		 for (int tmp=x;tmp!=Fx;tmp=f[tmp])++dhh3,H.dec(rank,w[tmp]);
		 for (int tmp=y;tmp!=zzy;tmp=f[tmp])++dhh3,H.dec(rank,w[tmp]);
		 for (int tmp=F[zzy][0];deep[tmp]>=deep[Fy];tmp=f[tmp])++dhh3,H.inc(rank,w[tmp]);
		 //printf("%d\n",ans);
		 continue;
	    }
