...
void dfs(int from,int x,int pre){
     ...
     if (X[x]){
        ...
        for (vector<int>::iterator i=son[x].begin();i!=son[x].end();++i){++dhh;
            if (!H.count(make_pair(tmp,*i)))H[make_pair(tmp,*i)]=p[*i];}
	...
	for (int wei=1;wei<=10;++wei)
    {
          ...
          if (deep[tmp]<deep[Fx]&&deep[tmp]<deep[Fy]){
             ...
             for (int tmp=x;tmp!=Fx;tmp=f[tmp]){
                 if (H[make_pair(rank,w[tmp])]==0)++ans;
                 H[make_pair(rank,w[tmp])]++;
                 }
             for (int tmp=y;tmp!=Fy;tmp=f[tmp]){
                 if (H[make_pair(rank,w[tmp])]==0)++ans;
                 H[make_pair(rank,w[tmp])]++;
                 }
             for (int tmp=x;tmp!=Fx;tmp=f[tmp])H[make_pair(rank,w[tmp])]--;
             for (int tmp=y;tmp!=Fy;tmp=f[tmp])H[make_pair(rank,w[tmp])]--;
             printf("%d\n",ans);continue;
             }
          if (Fx!=Fy){
             if (deep[Fx]<deep[Fy]){
             ...
             for (int tmp=x;tmp!=Fx;tmp=f[tmp])H[make_pair(rank,w[tmp])]--;
             for (int tmp=y;tmp!=zzy;tmp=f[tmp])H[make_pair(rank,w[tmp])]--;
             for (int tmp=F[zzy][0];deep[tmp]>=deep[Fy];tmp=f[tmp])H[make_pair(rank,w[tmp])]++;
             printf("%d\n",ans);continue;
          }
