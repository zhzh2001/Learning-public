var
  ...
  p,e,next,v,c,start,root,son,maxcost,fa,costfa,deep,tmp,ll,rr:array[0..30010]of longint;
  ch:char;
  ...
  procedure cdfs(x,ro:longint);
  var
    i:longint;
    begin
    if root[x]=x then maxcost[x]:=-10000000
      else begin
    ...
    while i<>0 do
      begin
      if (p[e[i]]=0)and(root[e[i]]=ro) then begin
        cdfs(e[i],ro);
        end;
      i:=next[i];
      end;
  ...
  for i:=1 to num do
    begin
    cdfs(start[i],start[i]);
    for j:=1 to tmp1 do p[tmp[j]]:=0;
    tmp1:=0;
    end;
  //writeln(deep[3]);
  while true do
    ...
    if ch='Q' then begin
      ...
      readln(x,y);
      c[x*2-1]:=y;c[x*2]:=y;
      cdfs(root[ll[x]],root[ll[x]]);
      for j:=1 to tmp1 do p[tmp[j]]:=0;
      tmp1:=0;
      cdfs(root[rr[x]],root[rr[x]]);
      for j:=1 to tmp1 do p[tmp[j]]:=0;
      tmp1:=0;
      end
