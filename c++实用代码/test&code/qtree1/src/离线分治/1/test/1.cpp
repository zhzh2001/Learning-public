...
2=Query the max weight of a path
*/
struct edge{
	node *T;
	Tnode *from,*link;
...
void contract(int L,int R,int dep){
	...
	/*for (int i=1;i<=t;++i)
		printf("nie %d %d %d %d\n",queue[i],queue[i]->p!=NULL?queue[i]->p->from:0,queue[i]->T->max,queue[i]->count);
	printf("\n\n");*/
	int top=0,Tmp1;
	for (int i=1;i<=t;++i)
	...
	for (int i1=1;i1<=t;++i1){
		...
		if (i->count>1){
			...
			i->Tnew->T->max=i->T->max;i->T->New=i->Tnew->T;
		}
	}
	for (int i1=1;i1<=t;++i1){
		Tnode *i=queue[i1];
		if (i->count>1&&i!=a[1]){
			node *tmp=&nodes[++N1];tmp->max=-oo;
			Tnode *j=i->p->from;
			while (j->count<=1){
				tmp->max=max(tmp->max,j->T->max);
				j=j->p->from;
			}
			j=i;
			do{
				if (j->p->T!=NULL)tmp->max=max(tmp->max,j->p->T->max);
				j=j->p->from;
			}while (j->count<=1);
			if (tmp->max!=-oo)addedge(j->Tnew,i->Tnew,tmp);
...
void release(int L,int R,int dep){
	...
	for (int i1=1;i1<=t;++i1){
		Tnode *i=queue[i1];
		if (i->count>1)i->T->max=i->T->New->max;
	}
	old1=data[dep].old1;
	for (int i=L;i<=R;++i)
		query[i]=old[++old1];
...
void Answer(int l,int r,int dep){
	if (l==r){
		//printf("t=%d %d %d\n",t);
		if (query[l].ch==1){
			query[l].x->T->max=query[l].z;
		}
		if (query[l].ch==2){
			int tmp;
			if (query[l].x->p!=NULL&&query[l].x->p->from==query[l].y)
				if (query[l].x->p->T!=NULL)
					tmp=max(query[l].x->T->max,query[l].x->p->T->max);
				else tmp=query[l].x->T->max;
			else if (query[l].y->p!=NULL&&query[l].y->p->from==query[l].x)
				if (query[l].y->p->T!=NULL)
					tmp=max(query[l].y->T->max,query[l].y->p->T->max);
				else tmp=query[l].y->T->max;
			else {
				tmp=max(query[l].x->T->max,query[l].y->T->max);
				if (query[l].x->p->T!=NULL)tmp=max(tmp,query[l].x->p->T->max);
				if (query[l].y->p->T!=NULL)tmp=max(tmp,query[l].y->p->T->max);
			}
			Ans[l]=tmp;
		}
		return;
