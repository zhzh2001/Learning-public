...
void Buildlca(int now,int next){
     int i,j,h=1,t=0,Max,top=n,dep,a1,Root;
     for (i=now;i<next;i++){
     ...
     while (h<t){
           ...
           a1=0;j=i;
           for (i=h;i<j;i++)if (!is[alca[i]]){is[alca[i]]=1;tmpa[++a1]=alca[i];}
           for (j=1;j<=a1;j++)alca[i-j]=tmpa[j];
           h=i-a1;
           }
     Root=alca[h];
     for (i=n;i;i--)if (is[a[i]]){
         node[a[i]].max=w[a[i]];
         if (a[i]==Root)continue;
         Max=-oo;j=f[a[i]];
         while (!is[j]){Max=max(Max,w[j]);j=f[j];}
	...
	while (T--){
          ...
          while (now<=q){
                ...
                for (i=now;i<next;i++){
                    ...
                    if (opt[i].ch=='B')printf("%d\n",Qmax(opt[i].x,opt[i].y));
                    }
                Rebuild();now=next;
                }
          }
