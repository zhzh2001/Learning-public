...
void Changex(int xx,char str[]){
	...
	for (int j=1;j<=n;j+=Lim){
		...
		for (int i1=x->y1;i1<=x->y2;++i1)
			delta=str[i1]-a[xx][i1],x->xsum[xx-x->x1]+=delta,x->ysum[i1-x->y1]+=delta,x->sum+=delta,a[xx][i1]=str[i1];
	}
}
void Changey(int yy,char str[]){
	...
	for (int i=1;i<=n;i+=Lim){
		Block *x=link[i][j];
		for (int i1=x->x1;i1<=x->x2;++i1)
			delta=str[i1]-a[i1][yy],x->xsum[i1-x->x1]+=delta,x->ysum[yy-x->y1]+=delta,x->sum+=delta,a[i1][yy]=str[i1];
	}
}
	...
	for (int i=1;i<=n;i+=Lim)
		for (int j=1;j<=n;j+=Lim){
			...
			if (x1>=xx1&&y1>=yy1&&x2<=xx2&&y2<=yy2)tmp+=link[i][j]->sum;
				...
				for (int i=max(x1,xx1);i<=min(x2,xx2);++i)
					for (int j=max(y1,yy1);j<=min(y2,yy2);++j)
						tmp+=a[i][j];
		}
	return tmp;
}
void print(){
	printf("start\n");
	for (int i=1;i<=n;++i){
		for (int j=1;j<=n;++j)printf("%c",a[i][j]+'A'-1);
		printf("\n");
	}
	printf("end\n");
}
